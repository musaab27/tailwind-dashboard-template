...
const USDCtoken = "0x04068DA6C83AFCFA0e13ba15A6696662335D5B75";

const contract = new Web3Client.eth.Contract(minABI, tokenAddress);
const mimContract = new Web3Client.eth.Contract(minABI, mimToken);
const USDCcontract = new Web3Client.eth.Contract(minABI, USDCtoken);
...
async function getBalance() {
 ...
 setAccounts(walletAddress);
  ...
  const USDCResult = await USDCcontract.methods.balanceOf(walletAddress).call(); 
    ...
    <div className="flex flex-wrap flex-col -m-4">
      ...
      <div className="sm:w-full p-4 ">
        <div className="border border-gray-200 p-6 rounded-lg bg-black">
          ...
          <h2 className="text-3xl text-white font-medium title-font mb-2">USDC Balance</h2>
          <p className="leading-relaxed text-white text-2xl">{Math.round(USDCBalance)} {"USDC"}</p>
        </div>
      </div>
      <div className="sm:w-full p-4">
        <div className="border border-gray-200 p-6 rounded-lg bg-black">
